...
MANIFEST="${TOP}/.repo/manifests/copperhead-private.xml"
BRANCH=$(grep "revision" "${MANIFEST}" | head -1 \
        | sed 's/^ *//g;s/revision=\"refs\/heads\///g;s/\".*//g')
STAGINGBRANCH="staging/${BRANCH}_${OPERATION}-${NEWTAG}"

# Build list of Copperhead forked repos
PROJECTPATHS=$(cat "${MANIFEST}" | sed '/Begin Copperhead/,$!d' | tail -n +2 | sed '/  <!-- Begin Copperhead specific projects -->/,$d' | sed -n 's/.*path="\([^"]\+\)".*/\1/p')
